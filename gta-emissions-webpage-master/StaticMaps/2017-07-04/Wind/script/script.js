...
legend.addTo(map);
  ...
  //Define a Function which will poll data from 'data_record_2017-07-04.txt'
  (function pollDataSource() {
//    setTimeout(function() {
      $.ajax({                        // This block does the bulk of the work:
        url: "../data_record_2017-07-04.txt",     // "Ajax" tells the browser to perform these tasks
        cache: false,                 // behind the scenes. If successful, the information
        success: function(data) {     // polled from data_record_2017-07-04.txt is passed to the processData
...
function processData(map, mapMarkers, data) {                               //  Here we define what happens to the data that got polled from data_record_2017-07-04.txt.
...
//format of data_record_2017-07-04.txt line format:
...
// co2dse,cod,codse,gasp,gaspse,t,tse,amb,ambse,rd1,rd1se,rd2,rd2se,avg time;        
        ...
        var latitude = dataComponents[1];                           // We set a variable for each parameter in data_record_2017-07-04.txt.
        ...
        var circleMarker = new L.circleMarker([latitude, longitude], {                      //  We Create a marker positioned and colored corresponding to the data passed
          color: getColor(temperature, dataArray),                                          //  from data_record_2017-07-04.txt.
          radius: 9,
          opacity: 0.9,
          fillOpacity: 0.9
          }).bindPopup(
